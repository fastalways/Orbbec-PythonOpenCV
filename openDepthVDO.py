...
# setting
set_fps = depth_stream.get(cv2.CAP_PROP_FPS)
set_fps = int(set_fps)
if(set_fps<=0): # if cannot load valid value -> will be fixed
    set_fps = 30

while(cv2.waitKey( int(1000 / set_fps))!=27):
    ...
    depth_ch1, depth_ch2, _  = cv2.split(depth_img) # 8upperbits data in ch1 / 8lowerbits data in ch2 / ignore ch3
    ...
    # --- DECODE DEPTH --- 8bits x 2ch -> to 16bits 1 ch
    ir_ch1, ir_ch2, _  = cv2.split(ir_img) # 8upperbits data in ch1 / 8lowerbits data in ch2 / ignore ch3
    ...
    fps = int(fps)
    cv2.putText(color_img, str(fps)+"fps", (7, 70), cv2.FONT_HERSHEY_SIMPLEX, 1, (20, 20, 0), 3, cv2.LINE_AA)
    ...
    cv2.imshow("Depth", decoded_depth)
    cv2.imshow("IR", decoded_ir)
